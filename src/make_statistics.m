...
targets = zeros(length(label), length(gt));
outputs = zeros(length(label), length(gt));
for ii = label;
    target = (gt == ii);
    output = (predicated == ii);
    targets(ii + 1, :) = target';
    outputs(ii + 1, :) = output';
end
my_plotroc(targets, outputs);

end


function out1 = my_plotroc(varargin)
      ...
      for plotcol=1:plotcols
        ...
        if (i<=numSignals)
          ...
          if (i==1) && (numClasses > 1)
            ...
            for ii=1:numClasses, labels{ii} = ['digit ' num2str(ii - 1)]; end
